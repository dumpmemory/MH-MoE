...
Batch = namedtuple("Batch", "ids src_tokens src_lengths constraints target")
...
def get_interactive_tokens_and_lengths(self, lines, encode_fn, tokenizer):
    """
    
    """
    tokens = []
    for src_str in lines:
        src_str = json.loads(src_str)['text']
        words = list(map(str, tokenizer.encode(src_str, allowed_special="all")))
        src_str = ' '.join(words)
        # if '▁' not in src_str and tokenizer is not None:
        #     words = tokenizer.encode(src_str, out_type=str)
        #     src_str = ' '.join(words)
        tokens.append(
...
def make_batches(lines, cfg, task, max_positions, encode_fn):
    ...
    # if os.path.exists('/mnt/msranlp/shumma/data/16g/sentencepiece.bpe.model'):
    #     tokenizer.Load('/mnt/msranlp/shumma/data/16g/sentencepiece.bpe.model')
    # elif os.path.exists('/mnt/unilm/shumma/data/16g/sentencepiece.bpe.model'):
    #     tokenizer.Load('/mnt/unilm/shumma/data/16g/sentencepiece.bpe.model')
    # else:
    #     tokenizer = None

    tokens, lengths = get_interactive_tokens_and_lengths(task, lines, encode_fn, tokenizer)
...
def eval_lm(
    ...
    print('| Interactive generation with the following args: max position', max_positions)
    ...
    # for inputs in buffered_read(cfg.interactive.input, 1):
        results = []
        ...
        logger.info("Loss: {}, ppl {}".format(avg_nll_loss, 2**avg_nll_loss))
        logger.info(
