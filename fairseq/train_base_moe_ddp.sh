...
OUTPUT_PATH=$OUTPUT_FOLDER/mhmoe_standard_code/base_moe_ddp/small-baseline-redstone_v2-flash_attn-${expert}experts
...
python -m torch.distributed.launch --nproc_per_node=16 --nnodes=4 --node_rank=$OMPI_COMM_WORLD_RANK --master_addr="$MASTER_IP" --master_port $PORT train.py /mnt/msranlp/shaohanh/data/redstone_v2_1_config/  \
...
--moe-gate-loss-wt 0.01 --moe-gate-loss-combine-method sum  --use-xmoe >> $OUTPUT_PATH/train.log