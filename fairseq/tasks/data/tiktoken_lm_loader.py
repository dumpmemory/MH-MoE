...
class TiktokenLmLoader(LMLoader):
    ...
    def _doc_jsonstr_to_ids(doc_jsonstr, spm_tokenizer=None, fs_dict=None):
        ...
        for line in filter(None, json.loads(doc_jsonstr)["text"].split("\n")):
            ...
            tokenized_tokens = TiktokenLmLoader.fs_encode_line(fs_dict, tokens, append_eos=False)
    ...
    @staticmethod
    def _doc_lines_to_ids(lines, spm_tokenizer=None, fs_dict=None):
        assert EOL_SYMBOL in fs_dict.indices
        eol_index = fs_dict.indices[EOL_SYMBOL]
        tokenized_ids = []
        for line in lines:
            if len(line) == 0 and len(tokenized_ids) > 0:
                # add eos index
                last_line_ids = tokenized_ids[-1]
                if last_line_ids[-1] == eol_index:
                    last_line_ids[-1] = fs_dict.eos_index
    ...
    def _read_from_files(self, source_file):
        ...
        if lines_to_ids:
            tokenized_ids = []
            try:
                ret = TiktokenLmLoader._doc_lines_to_ids(lines, spm_tokenizer=self.tokenizer, fs_dict=self.dictionary)
            ...
            for doc_jsonstr in lines:
                try:
                    ret = TiktokenLmLoader._doc_jsonstr_to_ids(doc_jsonstr, spm_tokenizer=self.tokenizer, fs_dict=self.dictionary)
                    tokenized_ids.extend(ret)
                except BaseException as e:
                    print(e)
                    print(doc_jsonstr)
