...
class Dictionary:
    ...
    # copied and modified based on `add_from_file()` fuction in this file
    def add_from_spm(self, f):
        """
        ...
        sp = spm.SentencePieceProcessor(model_file=f)
        tokens = [sp.id_to_piece(i) for i in range(len(sp))]
        indices_start_line = self._load_meta(tokens)
        # self.symbols = [] # clear dict to force token id to be the same as in the spm index
        # self.count = []
        # self.indices = {}
        for token in tokens[indices_start_line:]:
            try:
                word = token
                count = 1 # every token in the spm model appears only once
                overwrite = False 
                if word in self.indices:
                    continue
        ...
        # update special token index
        # self.bos_index = self.indices[self.bos_word]
        # if self.pad_word not in self.indices:
        #     self.add_symbol(self.pad_word, 1, False)
        # self.pad_index = self.indices[self.pad_word]
        # print(f'self.pad_index: {self.pad_index}; self.pad_word: {self.pad_word}')
