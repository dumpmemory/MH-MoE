...
from .fewshot_task import HarnessAnlir1, HarnessAnlir2, HarnessAnlir3, HarnessArc_challenge, HarnessArc_easy, HarnessBoolq, HarnessCopa, HarnessOpenbookqa, HarnessPiqa, HarnessRte, HarnessWic, HarnessWinogrande, HarnessHellaswag, HarnessRecord, HarnessTruthfullqaMC1, HarnessTruthfullqaMC2, HarnessSCIQ, HarnessLambada
from .fewshot_task import HarnessArc_challenge25s, HarnessHellaswag10s
import tiktoken, sentencepiece
...
task_map = {
    ...
    "harness_arc_challenge": HarnessArc_challenge, # using harness_eval criteria
    "harness_arc_easy": HarnessArc_easy, # using harness_eval criteria
    "harness_record": HarnessRecord,    # using harness_eval criteria
    "harness_truthfullqa_mc1": HarnessTruthfullqaMC1, # using harness_eval criteria
    "harness_truthfullqa_mc2": HarnessTruthfullqaMC2, # using harness_eval criteria
    "harness_arc_challenge_25s": HarnessArc_challenge25s, # using harness_eval criteria
    "harness_hellaswag_10s": HarnessHellaswag10s,
    "harness_sciq": HarnessSCIQ,
    "harness_lambada": HarnessLambada,
}

from .xnli_task import construct_tasks
xnli_tasks = construct_tasks()
task_map.update(xnli_tasks)
...
class FewshotEval(FairseqTask):
    ...
    def setup_task(cls, cfg, **kwargs):
        ...
        assert len(paths) > 0
        if len(cfg.spm_model) > 0 and len(cfg.dict_path) == 0:
            dictionary = Dictionary.load(cfg.spm_model, from_SPM=True)
            # dictionary.bos_index = 0
            # dictionary.unk_index = 1
            dictionary.add_symbol(EOL_SYMBOL)
        elif len(cfg.dict_path) > 0:
