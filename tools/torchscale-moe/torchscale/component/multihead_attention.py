...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        q = q.reshape(bsz * self.num_heads, tgt_len, self.head_dim).contiguous()
        k = k.reshape(bsz * self.num_heads, src_len, self.head_dim).contiguous()
        v = v.reshape(bsz * self.num_heads, src_len, self.head_dim).contiguous()
        ...
        if use_flash_attn and rel_pos is None and attn_mask is not None:
